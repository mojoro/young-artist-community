...
type RouteContext = {
  params: Promise<{ program_id: string; audition_id: string }>
}
...
export async function GET(_request: NextRequest, context: RouteContext): Promise<NextResponse> {
  try {
    ...
    if (!audition || audition.program_id !== program_id) {
      return notFound(`Audition with id '${audition_id}' not found.`)
    }

    return NextResponse.json(formatAudition(audition))
  } catch (err) {
    console.error('GET /programs/[program_id]/auditions/[audition_id] failed', err)
    return internalError('Failed to fetch audition.')
  }
}
...
export async function PUT(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  try {
    ...
    const existing = await prisma.audition.findUnique({
      where: { id: audition_id },
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (validated.instrument_ids !== null) {
        await tx.auditionInstrument.deleteMany({ where: { audition_id } })
        if (validated.instrument_ids.length > 0) {
          await tx.auditionInstrument.createMany({
            data: validated.instrument_ids.map((instrument_id) => ({
              audition_id,
              instrument_id,
            })),
          })
    ...
    const updated = await prisma.audition.findUnique({
      where: { id: audition_id },
      include: {
        location: true,
        audition_instruments: { include: { instrument: true } },
    ...
    if (!updated) {
      return internalError('Audition vanished after update.')
    }

    return NextResponse.json(formatAudition(updated))
  } catch (err) {
    console.error('PUT /programs/[program_id]/auditions/[audition_id] failed', err)
    return internalError('Failed to update audition.')
  }
}
